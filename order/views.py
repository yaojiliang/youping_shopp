...
def order_view(request):
    ...
    # 查看订单
        ...
        #           }
        orders = OrderInfo.objects.filter(user=user,status__lt=5)
        all_order = []
        for order in orders:
            o = {}
            o['id'] = order.id
    ...
    # 删除订单
        ...
        if not json_str:
            ...
            return JsonResponse(result)
        json_obj = json.loads(json_str.decode())
        good_id = json_obj['order_id']

        if not good_id:
            result = {'code': 10410, 'error': 'Please give me addr_id!'}
            return JsonResponse(result)
        try:
            order = OrderGoods.objects.get(id=good_id)
            order_info = OrderInfo.objects.filter(id=order.order_id).first()
            order_info.status=6
            order_info.save()
        ...
        try:
            ...
            order_info.status = 3
            order_info.save()
            result = {'code': 200, 'data': 'update successfully!'}
...
def order_pay_view(request):
    ...
    # 初始化
    alipay = AliPay(
        appid="2016101600702348",
        app_notify_url=None,  # 默认回调url
        # app_private_key_string=os.path.join(settings.BASE_DIR, '/order/app_private_key.pem'),
        app_private_key_string=open('/home/tarena/Aid1907/youping/order/app_private_key.pem','r+').read(),
        # 支付宝的公钥，验证支付宝回传消息使用，不是你自己的公钥,
        # alipay_public_key_string=os.path.join(settings.BASE_DIR, '/order/app_public_key.pem'),
        alipay_public_key_string=open('/home/tarena/Aid1907/youping/order/aplipay_public_key.pem','r+',).read(),
    ...
    order_string = alipay.api_alipay_trade_page_pay(
        ...
        subject='电子order_%d' % order_id,
        return_url='http://127.0.0.1:8080/user/%s'%user,
        notify_url='http://127.0.0.1:8080/user/%s'%user  # 可选, 不填则使用默认notify url
    )
...
def check_pay_view(request):
    ...
    alipay = AliPay(
        ...
        # 支付宝的公钥，验证支付宝回传消息使用，不是你自己的公钥,
        alipay_public_key_path=os.path.join(settings.BASE_DIR, 'order/aplipay_public_key.pem'),
        sign_type="RSA2",  # RSA 或者 RSA2
        debug=True  # 默认False
    )
